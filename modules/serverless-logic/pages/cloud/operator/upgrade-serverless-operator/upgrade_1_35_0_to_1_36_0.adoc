...
Every workflow with the `preview` profile must be deleted before applying the operator upgrade to OSL 1.36.0, and re-deployed after the upgrade is completed.
...
Every workflow with the `gitops` profile must be scaled to zero before applying the operator upgrade to OSL 1.36.0, and rescaled to 1 after the upgrade is completed.
...
[start=2]
. Some time after the OSL Operator 1.36.0 upgrade is completed, you’ll see that a new `ReplicaSet` for executing the Data Index 1.36.0 version is created.
+
All the Data Index replicasets belonging the OSL 1.35.0 must be deleted.
+
...
----
+
Following the example above, the old replicaset `sonataflow-platform-data-index-service-1111111111` must be deleted with the following command:
+
[source,terminal]
----
...
+
If your Data Index installation is configured to use a Knative Eventing Kafka Broker, to receive the workflow status change, and definition events, you must delete the `data-index-process-definition` `Trigger` created by the OSL 1.35.0 version.
...
----
+
After the command execution, the old `Trigger` will be deleted, and a new `Trigger` compatible with the OSL 1.36.0 version will be automatically created.
...
. Backup the Job Service database. (including the database objects, and not only the tables information)

*Post-operator upgrade steps:*

[start=2]
. Some time after the OSL Operator 1.36.0 upgrade is completed, you’ll see that a new `ReplicaSet` for executing the Job Service 1.36.0 version is created.
+
All the Job Service replicasets belonging the OSL 1.35.0 must be deleted.
+
You can see all the replicasets by executing a query like this:
+
[source,terminal]
...
----
+
Following the example above, the old replicaset `sonataflow-platform-jobs-service-1111111111` must be deleted with the following command:
+
[source,terminal]
----
