...
+
All the Data Index ReplicaSets belonging to the OSL 1.35.0 must be deleted.
+
...
+
Suppose your Data Index installation is configured to use a Knative Eventing Kafka Broker to receive the workflow status change and definition events. In that case, you must delete the `data-index-process-definition` `Trigger` created by the OSL 1.35.0 version.
...
+
All the Job Service ReplicaSets belonging to the OSL 1.35.0 must be deleted.
+
You can see all the ReplicaSets by executing a query like this:
+
[source,terminal]
