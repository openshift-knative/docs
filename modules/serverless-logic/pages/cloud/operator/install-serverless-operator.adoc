...
This guide describes how to install the {operator_name} in a Kubernetes or OpenShift cluster. The operator is in an xref:/cloud/operator/known-issues.adoc[early development stage] (community only) and has been tested on OpenShift {openshift_version_min}+ and link:{minikube_url}[Minikube].

.Prerequisites
* An OpenShift cluster with admin privileges. Alternatively, you can use Minikube or KIND.
...
.Install {product_name} Operator on Minikube
[source,shell,subs="attributes+"]
----
