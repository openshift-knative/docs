...
data:
  ...
  Dockerfile: "FROM {kogito_devservices_imagename}:latest AS builder\n\n#
...
----
In your editor, change the first line in the `Dockerfile` entry where it reads `FROM {kogito_devservices_imagename}:{operator_version}` to the desired image.
