...
== Installing the {product_name} Workflow CLI
...
.Procedure
. Create a variable which references the KN workflow plugin container image:
+
`export IMAGE_SWF_KN_TAG={osl_kn_cli_imagename}:{operator_version}`
+
. Pull the image using Docker or Podman
+
`podman pull $IMAGE_SWF_KN`
+
. Start the container image and get the ID of running container:
+
`export KN_CONTAINER_ID=$(docker run -di $IMAGE_SWF_KN)`
+
. Copy the binary of KN workflow plugin
+
...
. Unzip the archive:
+
`tar -xf kn-workflow-linux-amd64.tar.gz`
+
. Rename the archive to `kn-workflow`:
+
`mv kn-workflow-linux-amd64.tar.gz kn-workflow`
+
. Copy the `kn-workflow` binary file to a directory in your `PATH`, such as `/usr/local/bin`
...
--
. Use the `kn workflow` as follows:
+
--
