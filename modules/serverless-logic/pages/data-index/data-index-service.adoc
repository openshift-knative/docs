...
----
  data-index:
    ...
    image: quay.io/kiegroup/kogito-data-index-postgresql-nightly:{sonataflow_non_productized_image_tag} <1>
    ports:
      - "8180:8080"
    depends_on:
      postgres:
...
----
<1> Reference the right {data_index_ref} image to match with the type of Database, in this case `quay.io/kiegroup/kogito-data-index-postgresql-nightly:{sonataflow_non_productized_image_tag}`
