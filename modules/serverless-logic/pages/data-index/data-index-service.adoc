...
----
  data-index:
    ...
    image: {sonataflow_dataindex_postgresql_imagename}:latest <1>
    ports:
      - "8180:8080"
    depends_on:
      postgres:
...
<1> Reference the right {data_index_ref} image to match with the type of Database, in this case `{sonataflow_dataindex_postgresql_imagename}:latest`
