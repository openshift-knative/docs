...
[source, bash]
----
docker run -it -e JOBS_SERVICE_PERSISTENCE=postgresql -e VARIABLE_NAME=value quay.io/kiegroup/kogito-jobs-service-allinone-nightly:{sonataflow_non_productized_image_tag}
----
...
spec:
  containers:
    - name: jobs-service-postgresql
      image: quay.io/kiegroup/kogito-jobs-service-allinone-nightly:{sonataflow_non_productized_image_tag}
...
quay.io/kiegroup/kogito-jobs-service-allinone-nightly:{sonataflow_non_productized_image_tag}
----
