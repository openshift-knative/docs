...
[NOTE]
====
Currently, only `kie-addons-quarkus-persistence-jdbc` persistence add-on supports correlation. The `kie-addons-quarkus-persistence-jdbc`  add-on is configured for PostgreSQL. Other persistence add-ons will be supported in a future release.
====
